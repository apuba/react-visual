...
class DesignArea extends Component {
  ...
  deleteCurrentComponent(e) {
    ...
    this.props.editComponentId &&
      _.remove(this.props.dynamicComponentList, {
        id: this.props.editComponentId
      });
      ...
      return (
        <div
          className={
            'page_designer_item ' +
            col +
            (this.props.activeId === domId ? ' isactive' : '')
          }
          key={index}
          id={domId}
          style={style}
        ...
        >
          <Tooltip title="删除此组件">
            <Button
              ...
              shape="circle"
              className="actionBtn"
              onClick={this.deleteCurrentComponent.bind(this)}
            />
          </Tooltip>
          <label>{config.grid.label}</label>
          <span className="isComponent">
  ...
  render() {
    return (
      <div
        className={
          'draggable ant-row ' +
          (this.props.activeId === 'dom_0' ? 'isdroping' : '')
        }
        id="dom_0"
      ...
      >
        ...
        {!this.props.isDesign && (
          <div className="page_designer_tip">
            <span>请拖组件到这里进行设计</span>
          </div>
        )}
        {this.props.dynamicComponentList.map((component, index) =>
