...
class CodeEditorComp extends Component {    
    // 初始化组件
    initComponent() {
        const { updateBaseState,showCodeEditor, codeEditorLanguage,js ,css } = this.props;
        ...
        editorContainer.sendMessageToChild({type: 'SET_VALUE', value: defaultValue, language});
    }

    render() {
        const { css, showCodeEditor, language } = this.props;
        this.initComponent(css, language)
        return <div id="vseditor_box" style={{'display': (showCodeEditor ? 'block': 'none')}} data-state={showCodeEditor}>
...
export default connect(mapStateToProps, { updateBaseState })(CodeEditorComp)
